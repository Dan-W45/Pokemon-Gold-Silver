...
while True:
    ...
    player_tile_x=player_x//var.tilesize
    player_tile_y=player_y//var.tilesize
##    print(player_tile_x,player_tile_y)
    tx=ty=0
    for row in range(player_tile_y-5,player_tile_y+5):
        for column in range(player_tile_x-5,player_tile_x+5):
            try:
                    if row<0 or column<0: pass
                    else: g.blit(colours[tilemap[row][column]],(tx,ty))#((column*var.tilesize)+player_x,(row*var.tilesize)+player_y))
            except IndexError:
                    pass
            tx+=var.tilesize
        ty+=var.tilesize
